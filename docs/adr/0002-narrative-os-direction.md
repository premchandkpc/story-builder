...
- ✅ River workers refactored to DIP: all 6 workers depend on narrow service interfaces (SceneContextProvider, GenerationWriter, CharacterNamer, CharacterStateWriter, SummaryWriter, ValidationWriter, StoryFactory) instead of `*db.Queries`. Adapter layer in `internal/river/adapter.go` wraps `db.Queries` behind domain-typed interfaces.
